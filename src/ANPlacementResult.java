...
public class ANPlacementResult {
...
public static void SelectionSortNoDist(int [][] M, ArrayList<Integer> placed) throws IOException {
    ...
    public static void main(String[] args) throws IOException {
        ...
        cell = row.createCell(3);
        cell.setCellValue("Selection sort crossing");
        
        cell = row.createCell(4);
        cell.setCellValue("Selection sort no dist crossing");
        int [][]c;
        int [][]c_CCP;
        int [][]c_CCPMax;
        while(countRun<1000) {
           ...
           // System.out.println("Placed");
            ...
            //if((computeCrossing(c) <= computeCrossing(sortMatrix(H,c)) || computeCrossing(c) <= computeCrossing(sortMatrix(H4,c)))) {
            if(computeCrossing(c) <= computeCrossing(sortMatrix(H,c)) || computeCrossing(c) <= computeCrossing(sortMatrix(H4,c)) || computeCrossing(c) <= computeCrossing(sortMatrix(H5,c))) {
             continue; 
            }else {
                //l++;
            ...
            //for(int i = 0; i<n;i++)
             //   H2[i] = H2[i] + 1;
              ...
              cell = row.createCell(3);
              cell.setCellValue(computeCrossing(sortMatrix(H3,c)));
              
              cell = row.createCell(4);
              cell.setCellValue(computeCrossing(sortMatrix(H5,c)));
